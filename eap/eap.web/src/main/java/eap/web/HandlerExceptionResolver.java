...
//				}
				...
				return new ModelAndView(errorViewPrefix + "errorAjax");
			} else {
...
//				responseObject.setEvents(WebEventsHelper.getWebEvents(request).getEvents());
				mnv =  new ModelAndView(errorViewPrefix + "errorAjax");
			}
	...
	private ModelAndView getErrorModelAndView(HttpServletRequest request, int statucCode) {
		...
		return new ModelAndView(errorViewPrefix + String.format("error%d%s", statucCode, isAjaxRequest ? "Ajax" : ""));
	}

	public void setErrorViewPrefix(String errorViewPrefix) {
		this.errorViewPrefix = errorViewPrefix;
	}
}