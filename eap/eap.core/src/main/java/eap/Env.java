...
//	public static final String ENV_POPS_APP_MODE_DEV = "dev"; // development
	...
	private MapPropertySource loadEnvPropertiesPropertySource(String name, String path) throws IOException {
		...
		if (StringUtils.isNotBlank(path)) {
			try {
				envProperties.getSource().putAll(new ResourcePropertySource(new FileSystemResource(ResourceUtils.getFile(path))).getSource());
...
//		}
	...
	public void refresh(Map<String, String> envMap) {
		synchronized (this.getPropertySources()) {
			try {
				MapPropertySource envProperties = this.loadDefaultEnvPropertiesPropertySource(sourceName);
				logger.info("loaded env file: " + DEFAULT_ENV_CONFIG_PATH);
				
				envProperties.getSource().putAll(envMap);
				logger.info("loaded env data from envMap");
	...
	private MapPropertySource loadDefaultEnvPropertiesPropertySource(String name) throws IOException {
		...
		Resource[] resources = resourceLoader.getResources(DEFAULT_ENV_CONFIG_PATH);
